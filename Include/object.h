...
static inline void
_PyRef_UnpackShared(Py_ssize_t bits, Py_ssize_t *refcount, int *queued, int *merged);
...
static inline void Py_SET_REFCNT(PyObject *ob, Py_ssize_t refcnt) {
    _PyObject_SetRefcount(ob, refcnt);
}
#if !defined(Py_LIMITED_API) || Py_LIMITED_API+0 < 0x030b0000
...
PyAPI_FUNC(void) _Py_IncRefTotalN(Py_ssize_t);
PyAPI_FUNC(void) _Py_DecRefTotalN(Py_ssize_t);
PyAPI_FUNC(void) _Py_DecRefTotalObj(PyObject *op);
...
{
    return _Py_atomic_compare_uintptr_relaxed(&op->ob_tid, tid);
}
...
static inline int
_Py_REF_IS_MERGED(Py_ssize_t shared)
{
    return (shared & _Py_REF_SHARED_FLAG_MASK) == _Py_REF_MERGED;
}

static inline int
_Py_REF_IS_QUEUED(Py_ssize_t shared)
{
...
#endif
{
    ...
    _Py_DecRefTotal();
#endif
    if (_PY_LIKELY(tid == _Py_ThreadId())) {
#ifdef Py_REF_DEBUG
...
_PyRef_UnpackShared(Py_ssize_t bits, Py_ssize_t *refcount, int *queued, int *merged)
{
    *refcount = Py_ARITHMETIC_RIGHT_SHIFT(Py_ssize_t,
                                          bits,
...
_Py_RESURRECT(PyObject *op, Py_ssize_t refcnt)
{
    ...
    op->ob_tid = 0;
    op->ob_ref_local = 0;
    op->ob_ref_shared = (refcnt << _Py_REF_SHARED_SHIFT) + _Py_REF_MERGED;
}
...
{
    assert(!_PyObject_IS_IMMORTAL(op));
    Py_ssize_t old_shared = _Py_atomic_add_ssize(
        &op->ob_ref_shared,

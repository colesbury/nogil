...
PyFunction_NewWithBuiltins(PyObject *co, PyObject *globals, PyObject *builtins)
{
    ...
    if (code->co_nfreevars > 0) {
        func->freevars = PyObject_Calloc(code->co_nfreevars, sizeof(PyObject *));
...
static int
func_kwdefaults_to_dict(PyFunctionObject *op)
{
    PyCodeObject *co = _PyFunction_GET_CODE(op);
    Py_ssize_t num_kwargs = PyCode_NumKwargs(co);
    ...
    for (; i < op->num_defaults; i++, j++) {
        ...
        int err = PyDict_SetItem(kwdefs, name, value);
        if (err < 0) {
            Py_DECREF(kwdefs);
            return -1;
        }
    ...
    if (PyDict_Size(kwdefs) == 0) {
        Py_DECREF(kwdefs);
        return 0;
    }
    op->func_kwdefaults = kwdefs;
    return 0;
}
...
_PyFunction_GetKwDefaults(PyFunctionObject *op)
{
    if (op->func_kwdefaults == NULL) {
        if (func_kwdefaults_to_dict(op) < 0) {
            return NULL;
        }
    }
    Py_XINCREF(op->func_kwdefaults);
...
{
    Py_XINCREF(defaults);
    Py_XSETREF(op->func_kwdefaults, defaults);

    // clear keyword defaults stored in freevars
    PyCodeObject *co = _PyFunction_GET_CODE(op);
    Py_ssize_t num_kwargs = PyCode_NumKwargs(co);
    Py_ssize_t i = op->num_defaults - num_kwargs;
    for (; i < op->num_defaults; i++) {
        Py_CLEAR(op->freevars[i]);
    }
...
/*[clinic end generated code: output=99c6d9da3a24e3be input=93611752fc2daf11]*/
{
    ...
    nfree = code->co_nfreevars - code->co_ndefaultargs;
    ...
    newfunc = PyFunction_New((PyObject *)code, globals);
    if (newfunc == NULL) {
        return NULL;
    }
    ...
    if (name != Py_None) {
        Py_INCREF(name);
        Py_SETREF(((PyFunctionObject *)newfunc)->func_name, name);
    }
    if (defaults != Py_None) {
        Py_ssize_t num_defaults = PyTuple_GET_SIZE(defaults);
        if (_PyFunction_SetDefaults(newfunc, _PyTuple_ITEMS(defaults), num_defaults) != 0) {
            Py_DECREF(newfunc);
            return NULL;
        }
    }
    if (closure != Py_None) {
        if (PyFunction_SetClosure(newfunc, closure) != 0) {
            Py_DECREF(newfunc);
            return NULL;
        }
    }
...
func_set_code(PyFunctionObject *op, PyObject *value, void *Py_UNUSED(ignored))
{
    ...
    if (co->co_kwonlyargcount != prev->co_kwonlyargcount) {
        if (op->func_kwdefaults != NULL && func_kwdefaults_to_dict(op) < 0) {
            // convert kwdefaults to dict
            return -1;
        }
        Py_ssize_t old_kwargs = PyCode_NumKwargs(prev);
        Py_ssize_t num_kwargs = PyCode_NumKwargs(co);
        Py_ssize_t num_freevars = PyCode_NumFreevars(co);
        Py_ssize_t defcount = op->num_defaults - old_kwargs;
        Py_ssize_t new_size = defcount + num_kwargs + num_freevars;
        ...
        // copy over positional defaults and free variables. keyword-only defaults are
        // NULL because we've already converted keyword-only defaults to a dict.
        memcpy(freevars, op->freevars, defcount * sizeof(PyObject *));
        memcpy(freevars + num_kwargs, op->freevars + old_kwargs, num_freevars * sizeof(PyObject *));

        PyObject **prev = op->freevars;
        op->freevars = freevars;
        op->num_defaults = defcount + num_kwargs;

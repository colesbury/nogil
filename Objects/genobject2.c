...
PyGen2_NewWithCode(struct ThreadState *ts, PyCodeObject2 *co)
{
    if (co->co_flags & CO_COROUTINE) {
        return gen_new_with_qualname(&PyCoro2_Type, ts);
    }
    else if (co->co_flags & CO_ASYNC_GENERATOR) {
        return gen_new_with_qualname(&PyAsyncGen2_Type, ts);
    }
    else {
...
gen_dealloc(PyGenObject2 *gen)
{
    ...
    if (PyAsyncGen2_CheckExact(gen)) {
        /* We have to handle this case for asynchronous generators
           ...
           and GC_Del. */
        Py_CLEAR(((PyAsyncGenObject2 *)gen)->finalizer);
    }
    if (PyCoro2_CheckExact(gen)) {
        Py_CLEAR(((PyCoroObject2 *)gen)->origin);
    }
...
static PyMemberDef async_gen_memberlist[] = {
    {"ag_running", T_BOOL,   offsetof(PyAsyncGenObject2, running_async),
        READONLY},
    {"ag_code",    T_OBJECT, offsetof(PyGenObject2,  code),
        READONLY},
    {NULL}      /* Sentinel */
};

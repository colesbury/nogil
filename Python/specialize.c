...
static void
_py_set_opcode_failure(_Py_CODEUNIT *instr, uint8_t opcode_generic)
{
    if (!_PyRuntime.multithreaded) {
        _py_set_opcode(instr, _PyOpcode_Deopt[opcode_generic]);
    }
    else {
        _py_set_opcode(instr, opcode_generic);
    }
}
...
{
    if (_PyRuntime.multithreaded && instr->opcode != LOAD_ATTR) {
...
{
    if (_PyRuntime.multithreaded && instr->opcode != STORE_ATTR) {
        // another thread concurrently specialized this instruction
...
fail:
    STAT_INC(STORE_ATTR, failure);
    assert(!PyErr_Occurred());
    _py_set_opcode_failure(instr, STORE_ATTR_GENERIC);
    cache->counter = adaptive_counter_backoff(cache->counter);
    return;
success:
...
fail:
    STAT_INC(LOAD_GLOBAL, failure);
    assert(!PyErr_Occurred());
    _py_set_opcode_failure(instr, LOAD_GLOBAL_GENERIC);
    cache->counter = adaptive_counter_backoff(cache->counter);
    return;
success:
...
{
    if (_PyRuntime.multithreaded && instr->opcode != BINARY_SUBSCR) {
        // another thread concurrently specialized this instruction
...
fail:
    STAT_INC(BINARY_SUBSCR, failure);
    assert(!PyErr_Occurred());
    _py_set_opcode_failure(instr, BINARY_SUBSCR_GENERIC);
    cache->counter = adaptive_counter_backoff(cache->counter);
    return;
success:
...
{
    if (_PyRuntime.multithreaded && instr->opcode != STORE_SUBSCR) {
        // another thread concurrently specialized this instruction
...
fail:
    STAT_INC(STORE_SUBSCR, failure);
    assert(!PyErr_Occurred());
    _py_set_opcode_failure(instr, STORE_SUBSCR_GENERIC);
    cache->counter = adaptive_counter_backoff(cache->counter);
    return;
success:
...
_Py_Specialize_Call(PyObject *callable, _Py_CODEUNIT *instr, int nargs,
                    PyObject *kwnames)
{
    if (_PyRuntime.multithreaded && instr->opcode != CALL) {
        // another thread concurrently specialized this instruction
    ...
    if (fail) {
        STAT_INC(CALL, failure);
        assert(!PyErr_Occurred());
        _py_set_opcode_failure(instr, CALL_GENERIC);
        cache->counter = adaptive_counter_backoff(cache->counter);
    }
    else {
...
{
    if (_PyRuntime.multithreaded && instr->opcode != BINARY_OP) {
        // another thread concurrently specialized this instruction
    ...
    STAT_INC(BINARY_OP, failure);
    _py_set_opcode_failure(instr, BINARY_OP_GENERIC);
    cache->counter = adaptive_counter_backoff(cache->counter);
    return;
success:
...
_Py_Specialize_CompareOp(PyObject *lhs, PyObject *rhs, _Py_CODEUNIT *instr,
                         int oparg)
{
    if (_PyRuntime.multithreaded && instr->opcode != COMPARE_OP) {
        // another thread concurrently specialized this instruction
...
failure:
    STAT_INC(COMPARE_OP, failure);
    _py_set_opcode_failure(instr, COMPARE_OP_GENERIC);
    cache->counter = adaptive_counter_backoff(cache->counter);
    return;
success:
...
{
    if (_PyRuntime.multithreaded && instr->opcode != UNPACK_SEQUENCE) {
        // another thread concurrently specialized this instruction
...
failure:
    STAT_INC(UNPACK_SEQUENCE, failure);
    _py_set_opcode_failure(instr, UNPACK_SEQUENCE_GENERIC);
    cache->counter = adaptive_counter_backoff(cache->counter);
    return;
success:
...
{
    if (_PyRuntime.multithreaded && instr->opcode != FOR_ITER) {
        // another thread concurrently specialized this instruction
    ...
    STAT_INC(FOR_ITER, failure);
    _py_set_opcode_failure(instr, FOR_ITER_GENERIC);
    cache->counter = adaptive_counter_backoff(cache->counter);
    return;
success:
